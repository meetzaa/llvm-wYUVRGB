...
Value *CodeGenFunction::EmitRISCVBuiltinExpr(unsigned BuiltinID,
  ...
  /* case RISCV::BI__builtin_riscv_yuv_rgb: {
    llvm::Type *Int32Ty = llvm::Type::getInt32Ty(getLLVMContext());
    llvm::Type *PtrTy = CGM.Int8PtrTy; // i8*

    llvm::FunctionCallee Fn = CGM.getModule().getOrInsertFunction(
        "yuv_rgb_runtime", // wrapper C (pasul 3)
        Int32Ty, PtrTy, Int32Ty, Int32Ty);

    llvm::Value *Args[] = {EmitScalarExpr(E->getArg(0)),  // char*
                           EmitScalarExpr(E->getArg(1)),  // h
                           EmitScalarExpr(E->getArg(2))}; // w
    return Builder.CreateCall(Fn, Args, "yuv_rgb_call");
  }*/
  case RISCV::BI__builtin_riscv_yuvrgb: {
    llvm::Value *YUV = EmitScalarExpr(E->getArg(0)); // i8*
    llvm::Value *RGB = EmitScalarExpr(E->getArg(1)); // i8*
    llvm::Value *H32 = EmitScalarExpr(E->getArg(2)); // i32
    llvm::Value *W32 = EmitScalarExpr(E->getArg(3)); // i32
    llvm::Value *H = Builder.CreateZExt(H32, Builder.getInt64Ty());
    llvm::Value *W = Builder.CreateZExt(W32, Builder.getInt64Ty());

    llvm::Function *F = CGM.getIntrinsic(llvm::Intrinsic::riscv_yuvrgb);
    return Builder.CreateCall(F, {YUV, RGB, H, W});
    break;
  }
  case RISCV::BI__builtin_riscv_dot: {
    Value *A = EmitScalarExpr(E->getArg(0));
    Value *B = EmitScalarExpr(E->getArg(1));
    Value *C = EmitScalarExpr(E->getArg(2));
    Function *Intrinsic = CGM.getIntrinsic(Intrinsic::riscv_dot);
    return Builder.CreateCall(Intrinsic, {A, B, C});
    break;
  }
