...
SDValue RISCVTargetLowering::LowerINTRINSIC_W_CHAIN(SDValue Op,
  ...
  /*case Intrinsic::riscv_dot: {
    SDValue Chain = Op.getOperand(0);
    SDValue PtrA = Op.getOperand(2);
    SDValue PtrB = Op.getOperand(3);
    SDValue Count = Op.getOperand(4);

    // Construiește nodul cu lanț explicit
    SDVTList VTs = DAG.getVTList(Op.getValueType(), MVT::Other);
    SDValue Ops[] = {Chain, PtrA, PtrB, Count};

    MachineSDNode *Result = DAG.getMachineNode(RISCV::DOT, DL, VTs, Ops);
    return DAG.getMergeValues({SDValue(Result, 0), SDValue(Result, 1)}, DL);
  }*/ 
  default:
    break;
