...
#undef CASE_VMXOR_VMANDN_VMOR_OPCODES
  ...
  case ISD::INTRINSIC_W_CHAIN: {
    unsigned IntNo = Node->getConstantOperandVal(1);
    switch (IntNo) {
    case Intrinsic::riscv_dot: {
      SDValue Chain = Node->getOperand(0);
      SDValue PtrA = Node->getOperand(2);
      SDValue PtrB = Node->getOperand(3);
      SDValue Count = Node->getOperand(4);

      // 3 operanzi: PtrA, PtrB, Count + tipuri de return (rezultat,lant)
      MachineSDNode *Res = CurDAG->getMachineNode(
          RISCV::DOT, DL, {Node->getValueType(0), MVT::Other},
          {PtrA, PtrB, Count /*, Chain*/});
      ReplaceNode(Node, Res);
      return;
    }
  ...
  case ISD::INTRINSIC_VOID: {
    unsigned IntNo = Node->getConstantOperandVal(1);
    switch (IntNo) {
    case Intrinsic::riscv_yuvrgb: {
      SDLoc DL(Node);
      SDValue Chain = Node->getOperand(0);
      SDValue Y = Node->getOperand(2);
      SDValue R = Node->getOperand(3);
      SDValue H = Node->getOperand(4);
      SDValue W = Node->getOperand(5);

      auto *Res =
          CurDAG->getMachineNode(RISCV::YUVRGB, DL,
                                 MVT::Other,           // out-chain
                                 {Y, R, H, W, Chain}); // + in-chain last
      ReplaceNode(Node, Res);
      return;
    }
